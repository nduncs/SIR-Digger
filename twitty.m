...
%
% tw = twitty(credentials); % create a twitty object (twitty_obj) and supply user credentials.
%
...
%
% After valid credentials are set, twitty_obj should be able to access the Twitter. E.g.:  
...
% METHODS
% --------
% Conceptually, twitty methods are just wrapper functions around the main function which calls the
...
%                Default, twitty_obj.jsonParser = @ parse_json. 
%
...
%                However, if a twitty_obj connects to the Twitter Streaming API and no output
%                function was specified the output function would be set to:
%                   twitty_obj.outFcn = @ twitty_obj.displayStatusText.
%
...
%                Default, twitty_obj.sampleSize = 1000, i.e., 1000 statuses will be retrieved.
...
%                Default, twitty_obj.batchSize = 20.
%
...
%                  of the output function. See the 'twitty.plotStatusLocation()' function for an
%                  example of its usage. 
%
% =================
% 2. A JSON PARSER
% =================
% Response from the Twitter platform represents a string in the "JSON" format. The twitty class does
...
% twitty's property twitty_obj.jsonParser. 
% By default, 
%    twitty_obj.jsonParser = @ parse_json.
%
% NB: If no JSON parser is specified, twitty's methods would return just plain strings in JSON
%     format. 
%
% ==================================================
% 3. SEARCH API, STREAMING API and OUTPUT FUNCTIONS
...
%
% Via the Search API, a client (i.e., a twitty object) specifies a search query, sends it to the
...
%
% All but two twitty methods implement interfaces for accessing the Search API. 
...
% Alternatively, twitty can interrupt the connection automatically, after the desired number of
% tweets were retrieved. The property 'twitty_obj.sampleSize' controls this number. By default, 
%    twitty_obj.sampleSize = 1000.
% 
...
% facilitate this, the twitty class allows its users to provide a special function, the so called
% "ouput function", which job is to process the statuses as they are retrieved via the Streaming (or
% Search) API. 
% 
% A user can specify an output function by assigning a handle of the function to the twitty property
% 'outFcn', i.e., 
%    tw = twitty; tw.outFcn = @tweetingSummary.
% 
% An output function can a usual m-file, e.g., the "tweetingSummary.m" file provided with the
% twitty. Alternatively, it can be implemented as a twitty method. Several output functions are
% provided with the twitty as its methods. See, the cell AUXILIARY OUTPUT FUNCTIONS in this file. 
%
% An output function must have two arguments, e.g., tweetingSummary(twitty_obj, S), where
% twitty_obj is the twitty object itself, and S is a cell array of structures containing the
% retrieved statuses. The function doesn't have any output arguments. Instead, the results of the
% statuses analysis are stored in the special twitty variable, twitty_obj.data. This variable is a
...
% see the 'twitty.plotStatusLocation' function. The boolean variable 'outFcnInitialized' facilitates
% a one-time initialization of an output function. For an example of its usage, see the
% 'twitty.plotStatusLocation()' or the "tweetingSummary.m" functions.
% 
...
% twitty property 'batchSize' controls the size of a batch of statuses to collect for
...
% v1.1.0 (c) 2012-13, Vladimir Bondarenko <http://sites.google.com/site/bondsite> 
    ...
    properties(Access = private)
        credentials = '';
        ownOutFcn = 0; % 1 - if the output function is a twitty's method, 0 - if it is a user defined function. 
...
%% Auxiliary functions    
    methods 
        ...
        if isempty(twtr.jsonParserStr)
            warning('twitty:jsonParserUnspecified',...
                    ['No JSON parser specified.\n',...
                     'It is practical to use some parser, e.g., ''parse_json.m''.\n',...
                     'Otherwise, twitty''s methods will return plain JSON strings.\n']);
        elseif ~exist(twtr.jsonParserStr,'file')
            warning('twitty:jsonParserNotFound',...
                    ['The specified JSON parser, ' twtr.jsonParserStr ', was not found.\n',...
                     'twitty''s methods will return plain JSON strings.\n']);
        end
        ...
        %
        % Twitter returns the statuses as strings in JSON format. If a twitty user has provided a
        ...
        %  2) cell array of strings, otherwise.
            ...
            if exist(twtr.jsonParserStr,'file')
                ...
                try
                    ...
                    warning('twitty:ParseJsonError',[twtr.jsonParser ' error.\n' ME.message]);
                    C = ss;
                end
            else
                C = ss;
        ...
        %
        % Usage: l = twitty_obj.cell2list(c);
        % INPUT: 
        ...
        if isa(twtr.jsonParser,'function_handle') || isempty(twtr.jsonParser)            
            ...
            twtr.jsonParser = [];
            error('twitty:jsonParserWrongSetting',...
        ...
        if ~isa(twtr.outFcn,'function_handle') && ~isempty(twtr.outFcn)
            twtr.outFcn = [];  
            error('twitty:outputFunctionWrongSetting',...
        ...
        elseif isempty(twtr.outFcn) 
            ...
            % determine if the function is the twitty's own method or a user-defined function:
            funcName = strsplit(twtr.outFcnStr,'.');
            if length(funcName)==2 && ismethod('twitty',funcName{2})
                % output function is a twitty's method:
                twtr.ownOutFcn = 1;
            elseif length(funcName)==1 && ~exist(funcName{1},'file')
                error('twitty:outputFunctionNotFound',...
...
%% Timelines
        ...
        %
        % Usage: S = twitty_obj.mentionsTimeline(parKey1, parVal1, parKey2, parVal2, ...);
        % INPUT: 
        % (optional) 
        %        parKey?, 
        %        parVal? - parameters Key/Value pairs. For the complete list, 
        ...
        %                  or a json string.
        % Examples:
        % tw = twitty; S = tw.mentionsTimeline('include_entities','true');
...
%% Streaming
        ...
        % statuses. 
        %
        % Usage: S = twitty_obj.sampleStatuses();
        %             S = twitty_obj.sampleStatuses(parKey1, parVal1, parKey2, parVal2, ...);
        % INPUT:    
        % (optional) 
        %        parKey?, 
        %        parVal? - parameters Key/Value pairs. For the complete list, 
        ...
        % them. 
        % tw = twitty; tw.sampleSize = 1000; tw.batchSize = 100; S = tw.sampleStatuses;
            ...
            if isempty(twtr.outFcn)   % if the outFcn isn't set use the default one
                ...
                % issue a warning:
                warning('twitty:setDefaultOutputFunction',...
                'The output function was set to the default one: ''twitty_obj.displayStatusText''.');
            end
        ...
        %
        % Usage: S = twitty_obj.filterStatuses(parKey1, parVal1, parKey2, parVal2, ...);
        % INPUT:    
        ...
        % tw = twitty; tw.sampleSize = inf; tw.batchSize = 1; S = tw.filterStatuses('track','job');
...
%% Tweets
        ...
        %                  (requires a JSON parser), or a plain JSON string.
        % Examples:
        % tw = twitty; S = tw.updateStatus('hello twitter!','include_entities','true');
        ...
        %
        % Usage: S = twitty_obj.retweeters(id);
        %        S = twitty_obj.retweeters(id, parKey1, parVal1, parKey2, parVal2, ...);
        % INPUT: id      - a string, specifying the ID of the desired status;
        % (optional) 
        %        parKey?, 
        %        parVal? - parameters Key/Value pairs. For the complete list, 
        ...
        % Examples:
        % 1. tw = twitty; S = tw.retweeters('21947795900469248');
...
%% Search
        ...
        %% Search
        function S = search30day(twtr,query,varargin)
        % Search twitter.
        %
        % Usage: S = twitty_obj.search(query);
        %        S = twitty_obj.search(query, parKey1, parVal1, parKey2, parVal2, ...);
        ...
        % tw = twitty; S = tw.search('matlab','include_entities','true');
            ...
            % Call Twitter API:
            httpMethod = 'GET';
            url = 'https://api.twitter.com/1.1/tweets/search/30day/SIR.json';
            S = twtr.callTwitterAPI(httpMethod,url,params,1);
        end
                %% Search
        function S = searchfullarchive(twtr,query,varargin)
        % Search twitter.
        %
        % Usage: S = twitty_obj.search(query);
        %        S = twitty_obj.search(query, parKey1, parVal1, parKey2, parVal2, ...);
        ...
        % tw = twitty; S = tw.search('matlab','include_entities','true');
            ...
            % Call Twitter API:
            httpMethod = 'GET';
            url = 'https://api.twitter.com/1.1/tweets/search/fullarchive/SIR.json';
            S = twtr.callTwitterAPI(httpMethod,url,params,1);
        end
...
%% Friends & Followers
        ...
        %                  (requires a JSON parser, or a plain JSON string.
        % Examples:
        % tw = twitty; S = tw.friendshipsLookup('screen_name',{'twitterapi','twitter','matlab'});
...
%% Trends
        ...
        %
        % Usage: S = twitty_obj.trendsPlace(woeid);
        %        S = twitty_obj.trendsPlace(woeid, 'exclude', 'hashtags');
        ...
        % specified location.
        %
        % Usage: S = twitty_obj.trendsClosest(coord);
        % INPUT: (required) 
        ...
        %
        % Examples:
        % 1. tw = twitty; S = tw.trendsAvailable([53.5 -2.25]);
...
%                     theURL = URL([url '?' paramStr]);
        ...
        try
            ...
            error('twitty:connectionError', ME.message);
%             errStream = BufferedReader( InputStreamReader(httpConn.getErrorStream) );
        ...
        if ~isempty(s)
            ...
            if ~isempty(twtr.outFcnStr)
                % output function calls are different for twitty's own methods and user-defined
                % functions:
                if twtr.ownOutFcn
                    twtr.outFcn(S);
                else
...
%% Auxiliary output functions
    methods
        ...
        %
        % Usage: twitty_obj.displayStatusText(S)
        % INPUT:
        %  S - cell array of structures containing Twitter statuses.
        % OUTPUT: none. 
        %
        % Examples:
        %  tw=twitty; tw.outFcn=@tw.displayStatusText; S = tw.sampleStatuses;
        ...
        %
        % Usage: twitty_obj.displayStatusHashtags(S)
        % INPUT:
        %  S - cell array of structures containing Twitter statuses.
        % OUTPUT: none.
        % Examples:
        %  tw=twitty; tw.outFcn=@tw.displayStatusHashtags; S = tw.sampleStatuses;
        
            % Parse input:
            if length(S)==1 && isfield(S{1}, 'statuses')  % if S is returned by twitty.search(query)
        ...
        %
        % Usage: twitty_obj.plotStatusLocation(twtr, S)
        % INPUT: 
        %  S - cell array of structures containing Twitter statuses.
        % OUTPUT: none.
        %
        ...
        %
        % Examples: 
        %  tw = twitty; tw.outFcn=@tw.plotStatusLocation; tw.sampleSize=5000; tw.batchSize=1; 
        %  S = tw.sampleStatuses;
        ...
        %
        % Usage: twitty_obj.statsTwitterUsageByLanguage(twtr.S)
        % INPUT: 
        % S - cell array of structures of Twitter statuses.
        % OUTPUT: none. 
        %         The statistics is stored in the cell array twitty_obj.data.stats.
        %
        % Example:
        %   tw = twitty; tw.sampleSize = 1000; tw.batchSize = 20;
